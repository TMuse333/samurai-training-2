...
// Production props interface - all mandatory, no Partial
export interface AuroraImageHeroProdProps {
  // Text props (mandatory)
  textColor: string;
  baseBgColor: string;
  mainColor: string;
  ...
  description: string;
  buttonText: string;
  // BaseComponentProps items (optional in base, but we'll make them required if used)
  items?: never; // Not used in this component
  array?: never; // Not used in this component
}

const AuroraImageHero: React.FC<AuroraImageHeroProdProps> = (props) => {
  const {
    textColor,
    baseBgColor,
    mainColor,
