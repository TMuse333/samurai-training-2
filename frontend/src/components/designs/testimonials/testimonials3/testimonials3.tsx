...
export interface Testimonials3Props extends BaseComponentProps {
  title: string;
  description: string;
  testimonials: Array<{
    name: string;
...
const Testimonials3: React.FC<Testimonials3Props> = ({
  ...
  const colors = deriveColorPalette({
    ...
    baseBgColor,
    mainColor,
    bgLayout: bgLayout as GradientConfig,
  });

  const background = useAnimatedGradient(bgLayout as GradientConfig, colors);
